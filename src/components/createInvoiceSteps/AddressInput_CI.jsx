...
export default function AddressInput_CI(props) {
  ...
  const setConnectedWallet = async () => {
    const address = await imalConnect.getConnectedAddress();
    if (address) {
  ...
  return (
    <div>
      ...
      <TextField
        ...
        helperText={!isAddressValid ? 'This address is invalid' : null}/>
      <br/>
      <Button
        variant="contained"
        sx={{minHeight: 25, mt: 3}}
        onClick={setConnectedWallet}>
        My Vite Address (Connected Wallet)
      </Button>
    </div>
  );
}
