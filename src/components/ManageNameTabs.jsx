...
export default function ManageNameTabs(props) {
    ...
    return (
        <>
            <Typography variant="h4" component="div" color="error" sx={{ flexGrow: 1, textAlign: 'center'}} display={error === false ? "none" : "initial"}>An error occured: {error}</Typography>
            <Box sx={{ width: '100%' }}  component="div" display={statsLoaded === true && error === false ? "initial" : "none"}>
                ...
                {error === false ? namesArray.map((name, currentIndex) => (
                    <TabPanel value={value} index={currentIndex} key={name.nameId}>
                        <NamePanel name={name.name} nameId={name.nameId} owner={connectedAddress} isVerified={name.isTrusted} key={name.nameId} canManage={true}/>
                    </TabPanel>
                )) : null}
            </Box>
